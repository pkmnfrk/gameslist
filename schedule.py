...
class ListGame:
  def __init__(self, row):
    self.title = row[0]
    self.votes = None if len(row) <= 1 or int(row[1]) > 1000 else int(row[1])
    self.date_suggested = "2000-01-01" if len(row) <= 2 or not row[2] else row[2]
...
def write_game(f: TextIOWrapper, moby_game, game: ListGame):
  image_path = downloader.fetch_image(moby_game["cover"]) if moby_game["cover"] else None
  desc = moby_game["description"] if moby_game["description"] else ""
  f.write( "        <div class=\"game\">\n")
  if image_path:
    f.write(f"          <div class=\"realimage\" style=\"background-image: url({image_path})\"></div>\n")
  else:
    f.write(f"          <div class=\"fakeimage\">?</div>\n")
  f.write(f"          <h2>{moby_game['title']}</h2>\n")
  if game.votes:
    f.write(f"          <p class=\"votes\">Suggested by {game.attribution} on {game.date_suggested} <span class=\"votesreal\">({game.votes} vote{'' if game.votes == 1 else 's'})</span></p>")
    pass
  else:
    f.write(f"          <p class=\"votes\">Streamer chosen</p>")
...
def main():
  ...
  try:
    ...
    values = list(sorted(values, key=lambda g: ((10000-g.votes) if g.votes else 999999, g.date_suggested)))
